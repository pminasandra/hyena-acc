...
# This is done to ensure that print output is instantaneous on older consoles (which the author likes to use).
####################################################################################################################################################

# Following command will collect all functions defined in features.py and place it in a list.
list_of_feature_functions = [f[1] for f in getmembers(features)\
                                if type(f[1]) == types.FunctionType and\
                                f[1].__name__[0] != "_"]
